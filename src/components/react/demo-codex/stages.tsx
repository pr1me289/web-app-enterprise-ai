...
import { AuthorityBadge, TreatmentBadge, laneMeta } from './badges';
  ...
  if (prevented) {
    return (
      <StageShell state="pending" icon={Database} eyebrow="Stage 1" title="Source-aware retrieval">
        <PreventedNote message={step.notRunReason ?? 'Step did not run; no sources queried.'} />
      </StageShell>
    );
  }
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 font-mono text-[0.6rem] uppercase tracking-[0.1em] ${
                        passed
                          ? 'border border-spruce-700/30 bg-paper text-spruce-700'
                          : 'border border-accent/40 bg-paper text-accent'
                      }`}
                    >
