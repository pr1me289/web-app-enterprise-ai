...
6. For each step, the Supervisor issues subqueries in the order listed, unless `subquery_condition` evaluates to false — in that case, skip and log the skip reason.
7. After all subqueries for a step complete, the Supervisor assembles the context bundle per `bundle_assembly_priority` and passes it to the assigned agent.
8. The Supervisor writes an audit log entry for every retrieval attempt, every chunk admitted or excluded, every determination, and every status change.
